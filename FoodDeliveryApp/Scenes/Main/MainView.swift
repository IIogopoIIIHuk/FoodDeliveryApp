import SwiftUI

// MARK: - Модели данных
struct MenuItem: Identifiable, Hashable {
    let id = UUID()
    let category: String
    let name: String
    let description: String
    let price: Int
    let imageName: String
}

struct PromoBanner: Identifiable {
    let id = UUID()
    let imageName: String
}

// MARK: - Данные
fileprivate let categories: [String] = ["Пицца", "Комбо", "Десерты", "Напитки", "Салаты", "Закуски"]

fileprivate let promoBanners: [PromoBanner] = [
    PromoBanner(imageName: "discount-30"),
    PromoBanner(imageName: "birthday-promo"),
]

fileprivate let menuItems: [MenuItem] = [
    MenuItem(category: "Пицца", name: "Ветчина и грибы", description: "Ветчина, шампиньоны, увеличенная порция моцареллы, томатный соус", price: 345, imageName: "Bufallo"),
    MenuItem(category: "Пицца", name: "Баварские колбаски", description: "Баварские колбаски,ветчина,пикантная пепперони,острая чоризо,моцарелла,томатный соус", price: 345, imageName: "Bayern"),
    MenuItem(category: "Пицца", name: "Нежный лосось", description: "Лосось, томаты черри, моцарелла, соус песто", price: 345, imageName: "Marino"),
    
    MenuItem(category: "Комбо", name: "Комбо 1", description: "Пицца, напиток и десерт", price: 599, imageName: "combo_1"),
    MenuItem(category: "Комбо", name: "Комбо 2", description: "2 пиццы, 2 напитка", price: 899, imageName: "combo_2"),
    
    MenuItem(category: "Десерты", name: "Чизкейк", description: "Классический чизкейк с вишнёвым топпингом", price: 250, imageName: "dessert_1"),
    MenuItem(category: "Десерты", name: "Тирамису", description: "Классический итальянский десерт", price: 280, imageName: "dessert_2"),
    
    MenuItem(category: "Напитки", name: "Кола", description: "Газированный напиток 0.5л", price: 100, imageName: "drink_1"),
    MenuItem(category: "Напитки", name: "Сок", description: "Апельсиновый сок 1л", price: 150, imageName: "drink_2")
]

fileprivate let menuDictionary = Dictionary(grouping: menuItems, by: { $0.category })

// MARK: - Главная View (с полной логикой)

struct MainView: View {
    @EnvironmentObject var appStateManager: AppStateManager
    
    @State private var showSuccessBanner: Bool = false
    @State private var selectedCategory: String? = "Пицца"
    
    @State private var selectedTab: Tab = .menu
    @State private var scrollOffset: CGFloat = 0

    enum Tab: String {
        case menu, contacts, profile, cart
    }
    
    var body: some View {
        NavigationView {
            ZStack(alignment: .bottom) {
                Color.black.ignoresSafeArea() // Используем цвет из Assets
                
                switch selectedTab {
                case .menu:
                    MenuScreenContent(
                        showSuccessBanner: $showSuccessBanner,
                        appStateManager: appStateManager,
                        selectedCategory: $selectedCategory
                    )
                case .contacts:
                    Text("Контакты")
                case .profile:
                    ProfileView()
                case .cart:
                    Text("Корзина")
                }
                
                CustomTabBarView(selectedTab: $selectedTab)
            }
            .navigationTitle(navigationTitleForTab)
            .navigationBarTitleDisplayMode(.inline)
            .navigationBarItems(leading:
                Button(action: {
                    // Логика выбора города
                }) {
                    HStack(spacing: 4) {
                        Text("Москва")
                            .font(.system(size: 17))
                            .fontWeight(.semibold)
                            .foregroundColor(.black)
                        Image(systemName: "chevron.down")
                            .font(.system(size: 12))
                            .foregroundColor(.black)
                    }
                }
            )
            .navigationBarBackButtonHidden(true)
            .onAppear {
                if appStateManager.mainScreenMessage != nil {
                    showSuccessBanner = true
                    DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                        if showSuccessBanner {
                            showSuccessBanner = false
                            appStateManager.mainScreenMessage = nil
                        }
                    }
                }
            }
        }
    }
    
    private var navigationTitleForTab: String {
        switch selectedTab {
        case .menu:
            return ""
        case .contacts:
            return "Контакты"
        case .profile:
            return "Профиль"
        case .cart:
            return "Корзина"
        }
    }
}


// MARK: - Вспомогательные View

struct MenuScreenContent: View {
    @Binding var showSuccessBanner: Bool
    @ObservedObject var appStateManager: AppStateManager
    @Binding var selectedCategory: String?
    
    @State private var headerOffset: CGFloat = 0

    var body: some View {
        ScrollViewReader { proxy in
            ZStack(alignment: .top) {
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        if showSuccessBanner, let message = appStateManager.mainScreenMessage {
                            SuccessBannerView(message: message) {
                                showSuccessBanner = false
                                appStateManager.mainScreenMessage = nil
                            }
                            .padding(.horizontal)
                            .transition(.move(edge: .top))
                            .animation(.default, value: showSuccessBanner)
                        }
                        
                        PromotionsView()
                            .padding(.top, 16)
                        
                        CategoryBarView(
                            categories: categories,
                            selectedCategory: $selectedCategory,
                            scrollToCategory: { category in
                                withAnimation {
                                    proxy.scrollTo(category, anchor: .top)
                                }
                            }
                        )
                        .background(
                            GeometryReader { geometry -> Color in
                                let offset = geometry.frame(in: .global).minY
                                DispatchQueue.main.async {
                                    self.headerOffset = offset
                                }
                                return Color.clear
                            }
                        )
                        .padding(.top, 24)
                        
                        MenuContentSection(selectedCategory: $selectedCategory, proxy: proxy)
                            .padding(.top, 24)
                            .padding(.bottom, 80)
                    }
                }
                .background(Color("backgroundMain"))
                .padding(.top, headerOffset < 110 ? 50 : 0)
            }
            
            if headerOffset < 110 {
                VStack(spacing: 0) {
                    CategoryBarView(
                        categories: categories,
                        selectedCategory: $selectedCategory,
                        scrollToCategory: { category in
                            withAnimation {
                                proxy.scrollTo(category, anchor: .top)
                            }
                        }
                    )
                    .background(Color.white)
                    .shadow(radius: 2)
                    .transition(.move(edge: .top))
                }
            }
        }
    }
}

struct PromotionsView: View {
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 16) {
                ForEach(promoBanners) { banner in
                    Image(banner.imageName)
                        .resizable()
                        .aspectRatio(contentMode: .fit)
                        .frame(width: 300, height: 112)
                        .cornerRadius(10)
                }
            }
            .padding(.horizontal)
        }
    }
}

struct CategoryBarView: View {
    let categories: [String]
    @Binding var selectedCategory: String?
    let scrollToCategory: (String) -> Void
    
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ForEach(categories, id: \.self) { category in
                    Button(action: {
                        selectedCategory = category
                        scrollToCategory(category)
                    }) {
                        Text(category)
                            .font(.system(size: 13))
                            .fontWeight(selectedCategory == category ? .bold : .regular)
                            .padding(.vertical, 8)
                            .padding(.horizontal, 23)
                            .background(selectedCategory == category ? Color("pinkButton").opacity(0.2) : Color.white)
                            .foregroundColor(selectedCategory == category ? Color("pinkButton") : Color("pinkButton").opacity(0.4))
                            .cornerRadius(20)
                            .overlay(
                                RoundedRectangle(cornerRadius: 20)
                                    .strokeBorder(selectedCategory == category ? Color.clear : Color("pinkButton").opacity(0.2))
                            )
                    }
                }
            }
            .padding(.horizontal)
        }
    }
}

struct MenuContentSection: View {
    @Binding var selectedCategory: String?
    let proxy: ScrollViewProxy
    
    var body: some View {
        VStack(spacing: 0) {
            ForEach(categories, id: \.self) { category in
                if let items = menuDictionary[category] {
                    VStack(alignment: .leading, spacing: 16) {
                        Text(category)
                            .font(.system(size: 24, weight: .semibold))
                            .foregroundColor(.black)
                            .padding(.leading, 16)
                            .id(category)
                        
                        VStack(spacing: 16) {
                            ForEach(items) { item in
                                MenuItemView(item: item)
                                if item.id != items.last?.id {
                                    Divider()
                                        .padding(.leading, 160)
                                }
                            }
                        }
                    }
                    .padding(.top, 24)
                }
            }
        }
    }
}

struct MenuItemView: View {
    let item: MenuItem
    
    var body: some View {
        HStack(alignment: .top, spacing: 16) {
            Image(item.imageName)
                .resizable()
                .aspectRatio(contentMode: .fit)
                .frame(width: 132, height: 132)
                .cornerRadius(10)
            
            VStack(alignment: .leading, spacing: 8) {
                Text(item.name)
                    .font(.system(size: 16, weight: .semibold))
                    .foregroundColor(.black)
                
                Text(item.description)
                    .font(.system(size: 13))
                    .foregroundColor(.gray)
                    .lineLimit(nil)
                
                Spacer()
                
                HStack {
                    Spacer()
                    Button(action: {
                        // Логика добавления в корзину
                    }) {
                        Text("от \(item.price) р")
                            .font(.system(size: 13, weight: .semibold))
                            .foregroundColor(Color("pinkButton"))
                            .padding(.vertical, 8)
                            .padding(.horizontal, 16)
                            .background(Color.white)
                            .cornerRadius(6)
                            .overlay(
                                RoundedRectangle(cornerRadius: 6)
                                    .stroke(Color("pinkButton"), lineWidth: 1)
                            )
                    }
                }
            }
        }
        .padding(16)
        .frame(height: 160)
        .background(Color.white)
        .cornerRadius(10)
    }
}

struct CustomTabBarView: View {
    @Binding var selectedTab: MainView.Tab
    
    var body: some View {
        VStack(spacing: 0) {
            Divider()
            HStack {
                Spacer()
                TabBarButton(iconName: "menu-icon", label: "Меню", isSelected: selectedTab == .menu) {
                    selectedTab = .menu
                }
                Spacer()
                TabBarButton(iconName: "contacts-icon", label: "Контакты", isSelected: selectedTab == .contacts) {
                    selectedTab = .contacts
                }
                Spacer()
                TabBarButton(iconName: "profile-icon", label: "Профиль", isSelected: selectedTab == .profile) {
                    selectedTab = .profile
                }
                Spacer()
                TabBarButton(iconName: "trash-icon", label: "Корзина", isSelected: selectedTab == .cart) {
                    selectedTab = .cart
                }
                Spacer()
            }
            .frame(height: 80)
            .background(Color.white)
        }
        .edgesIgnoringSafeArea(.bottom)
    }
}

struct TabBarButton: View {
    let iconName: String
    let label: String
    let isSelected: Bool
    let action: () -> Void
    
    var body: some View {
        Button(action: action) {
            VStack(spacing: 4) {
                Image(iconName)
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .frame(width: 24, height: 24)
                Text(label)
                    .font(.system(size: 10))
            }
            .foregroundColor(isSelected ? Color("pinkButton") : .gray)
        }
    }
}

struct SuccessBannerView: View {
    let message: String
    let onClose: () -> Void
    
    var body: some View {
        HStack {
            Spacer()
            Image(systemName: "checkmark.circle.fill")
                .foregroundColor(.green)
            Text(message)
                .font(.system(size: 13))
                .fontWeight(.semibold)
                .foregroundColor(.black)
            Spacer()
        }
        .padding(.vertical, 16)
        .padding(.horizontal, 16)
        .background(Color.white)
        .cornerRadius(20)
        .frame(width: 343, height: 50)
        .shadow(color: Color.black.opacity(0.1), radius: 5, x: 0, y: 2)
        .padding(.horizontal)
        .padding(.top, 8)
    }
}

struct MainView_Previews: PreviewProvider {
    static var previews: some View {
        MainView()
            .environmentObject(AppStateManager())
    }
}
